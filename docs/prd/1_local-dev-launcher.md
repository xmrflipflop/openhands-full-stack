...
| Path | Role |
| --- | --- |
| `ecosystem.config.js` | The single committed PM2 process ecosystem (workspace-owned). Derives role, identity, app names, ports, namespace, and named runtime environments from the checkout location and a per-checkout id; defines the uv-venv interpreter and supervision limits. No secrets or env-specific values are hard-coded in app code. |
...
- **FR1** — A single committed `ecosystem.config.js` at the repo root defines every app. It derives role, identity, app names, ports, namespace, and runtime environment from the checkout location and a per-checkout id at evaluation time, so `pm2 start ecosystem.config.js` requires no per-invocation flags to produce a correct, clash-free deployment.
- **FR2** — Role comes from the checkout path. A checkout located under `/opt` is **prod** (exactly one, fixed base ports). Any other checkout location is **dev**. The role selects the named runtime environment and the port block.
...
- **FR10** — Named runtime environments `env` / `env_staging` / `env_production` are provided and selected with `--env <name>`. Prod defaults to `env_production`; dev defaults to `env`. Environment-specific values (ports, environment names, credential references) live only in the ecosystem, never in application code.
...
- **FR13** — Day-to-day operations use PM2 directly: `pm2 start ecosystem.config.js [--env <name>]` to launch, `pm2 status` to list (grouped by namespace), `pm2 restart <namespace>` / `pm2 stop <namespace>` for whole instances, `pm2 logs <app-name>` for one process. `pm2 save` snapshots the running set and `pm2 resurrect` restores it across machine restarts.
- **FR14** — `just dev` is a thin wrapper that forwards to `pm2 start ecosystem.config.js` (passing `--env` through). It is not a supervisor and adds no process-management logic of its own; all supervision is PM2's.
