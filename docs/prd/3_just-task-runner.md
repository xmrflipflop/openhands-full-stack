...
- **FR2a** — `setup` bootstraps the dependencies the stack expects (allocates the per-checkout `.dev-id` for every checkout — production included — runs `uv sync` for the SDK and for the automation package, and `npm install` for the frontend; the automation package keeps its own venv because it pins its `openhands-sdk` from PyPI rather than resolving the SDK workspace members editable). It additionally accepts a `--production` flag (`[arg("production", value="true")] setup production="false"`) that, when set, builds the Agent Canvas production bundle (the artifact the production launcher serves); without it the build is skipped so dev-mode checkouts pay no build cost. This is the one workspace task that is aware of mode: it produces the artifact consumed by the production serving path (`docs/prd/1_local-dev-launcher.md` FR8 prod, FR8c). The flag is named `--production` to match the launcher's and `just serve`'s `--production`, so the surface is uniform. It is the only flag `setup` honours; other arguments are ignored.
- **FR2b** — In dev mode (no `--production`) `setup` additionally runs the upstream-remote configuration step (`just setup-remotes`, which records the `OpenHands`, `software-agent-sdk`, and `automation` remote URLs idempotently and touches no network), so a fresh checkout is ready to `just sync` against the upstream subtrees without a separate command. The step is conditional on mode: it runs only when `--production` is absent, because a production deployment does not need the subtree-sync remotes. It is a wrapper call (`just setup-remotes`), keeping a single source of truth for the remote configuration and staying within NFR1's linear-conditional allowance (the conditional is the exception the inline-shortcut relaxation grants, the same mechanism FR2a's build step uses).
...
- **`just setup-remotes` adjusted — automatic in dev, skipped in production.** Considered two options for ensuring a fresh checkout has the upstream subtree remotes: require the contributor to run `just setup-remotes` separately, or fold it into `just setup`. Folding in was chosen because a fresh clone should reach a `just sync`-ready state from `just setup` alone, and the step is cheap (recording URLs, no fetch), idempotent, and offline-safe. It is gated on the absence of `--production`: production deployments serve a built stack and have no reason to add the subtree-sync remotes (and may run on hosts where git remotes are irrelevant or read-only). Implemented as a `just setup-remotes` call inside the dev branch of `setup`'s conditional rather than inlining the `git remote add`/`set-url` sequence, so the remote-configuration recipe stays the single source of truth (and stays directly runnable). The same NFR1 inline-conditional exception that covers FR2a's build step covers this one-line addition.
