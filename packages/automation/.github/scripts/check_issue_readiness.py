...
- Bug reports (labeled `bug`): the Actual Behavior section must describe a
  reproducible automation service run and include a supported command
  (`uv`, `pytest`, `python`, or `curl`), plus a non-empty Acceptance Criteria
...
# A reproducible command from this repository's supported development workflow
# (see AGENTS.md: `uv run pytest`, `uv run pre-commit`, service API via `curl`)
# must appear in the Actual Behavior section.
...
def check_bug(sections: dict[str, str]) -> ReadinessResult:
    ...
    if not actual:
        ...
        result.add(
            ...
            "from the automation development workflow, such as `uv`, `pytest`, "
            "`python`, or `curl`."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--labels",
        help="Comma-separated issue labels (e.g. 'bug,enhancement').",
        default="",
    )
    parser.add_argument(
        "--event-path",
